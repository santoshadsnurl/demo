...
if(isset($_REQUEST['searchQuery']) && !empty($_REQUEST['searchQuery']))
{
	...
	$strQuery = "WHERE pin_code LIKE '%$searchInput%'";
}

if(isset($_GET['id']) && $_GET['id']!="")
{
	$id = $_GET['id'];
	$category_id = $newobject->getdata($conn,"shipping_area","category_id","id",$id);
	$query_delete="DELETE FROM shipping_area WHERE id='".$id."'";
	if($sql_delete=$conn->query($query_delete))
	{
		$query_delete_sub="DELETE FROM sub_shipping_area WHERE category_id='".$category_id."'";
		$sql_delete_sub=$conn->query($query_delete_sub);
		$query_delete_sub_sub="DELETE FROM sub_sub_shipping_area WHERE category_id='".$category_id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM shipping_area WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM shipping_area $strQuery ORDER BY id ASC"))
{
...
$query="SELECT * FROM shipping_area $strQuery ORDER BY id ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
		...
		<div class="main-section">
			...
			<div class="col-sm-12 col-md-12 col-xs-12">
				<div class="panel panel-default">
					...
					<h3 class="panel-title">Shipping Area</h3>
					</div>
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px;" type="text" name="searchQuery" class="form-control" placeholder="Search By Category" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="shipping_area.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<input type="checkbox" onclick="return chkstatus('shipping_area','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('shipping_area','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="shipping_area_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="shipping_area.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM shipping_area $strQuery ORDER BY id ASC $limitvar";
