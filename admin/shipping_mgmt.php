...
{
	$id = $_GET['id'];
	$sub_category_id = $newobject->getdata($conn,"shipping","sub_category_id","id",$id);
	$query_delete="DELETE FROM shipping WHERE id='".$id."'";
	if($sql_delete=$conn->query($query_delete))
	{
		
		$query_delete_sub_sub="DELETE FROM sub_shipping WHERE sub_category_id='".$sub_category_id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM shipping WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM shipping $strQuery ORDER BY id ASC"))
{
...
$query="SELECT * FROM shipping $strQuery ORDER BY id ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							...
							<div class="col-sm-4">
								<a href="shipping.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
						...
						<table class="table table-bordered" style="margin-top:15px;">
							...
							if($sql_select=$conn->query($query))
							{
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td><?php echo $result['shipping_carge']; ?></td>
											<td><?php echo $result['price_upto']; ?></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('shipping','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('shipping','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="shipping_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="shipping.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
								...
								{
									?>
									<td  colspan="6" align="center"><font color="red" shipping="2">Record Not Found</font></td>
									<?php
								}
							}
							?>
...
<?php  
$ban="SELECT * FROM shipping $strQuery ORDER BY id ASC $limitvar";
