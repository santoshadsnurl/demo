...
{
	$id = $_GET['id'];
	$category_id = $newobject->getdata($conn,"discount","category_id","id",$id);
	$query_delete="DELETE FROM discount WHERE id='".$id."'";
	if($sql_delete=$conn->query($query_delete))
	{
		$query_delete_sub="DELETE FROM sub_discount WHERE category_id='".$category_id."'";
		$sql_delete_sub=$conn->query($query_delete_sub);
		$query_delete_sub_sub="DELETE FROM sub_sub_discount WHERE category_id='".$category_id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM discount WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM discount $strQuery ORDER BY id ASC"))
{
...
$query="SELECT * FROM discount $strQuery ORDER BY id ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px;" type="text" name="searchQuery" class="form-control" placeholder="Search By Code" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="discount.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td><?php echo $result['dist_rate']; ?></td>
											<td><?php echo $result['startdate']; ?></td>
											<td><?php echo $result['enddate']; ?></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('discount','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('discount','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="discount_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="discount.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM discount $strQuery ORDER BY id ASC $limitvar";
