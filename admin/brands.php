...
function resizeImg($imgsrc, $imgdest, $maxW='*', $maxH='*', $allowScaleUp=0, $returnHTML="alt='image'")
{
	if($s=getimagesize($imgsrc))
	{
		...
		if(preg_match('/\.(png)$/i', $imgsrc) || isPNG($imgsrc))
		{
			$imageTmp = imagecreatefrompng($imgsrc);
		}
		else
		{
			//$image = imagecreatefromjpeg($filename);
			$imageTmp = imagecreatefromjpeg($imgsrc);
		}
		// copy and resize old image into new image
		imagecopyresized($tmp_img, $imageTmp, 0, 0, 0, 0, $maxW, $maxH, $oW, $oH );
		// save thumbnail into a file
		imagejpeg($tmp_img,$imgdest);
		//chmod($imgsrc, 0777);
		//return ($returnHTML ? "<img src='abc.jpg' />" : array(0=>$w,1=>$h,"width"=>$w,"height"=>$h));
	}
	else
	{
...
if(isset($_POST['submit']) && $_POST['submit']=="add")
{
	...
	$query_duplicate="SELECT * FROM brands WHERE (title='".$title."' OR id='".$id."') $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		else
		{
			if($id!="")
			{
				$query_update="UPDATE brands SET title='".$title."',arabic_title='".$arabic_title."',arabic_meta_title='".$arabic_meta_title."',brand_id='".$brand_id."',order_no='".$order_no."',alias='".$alias."',meta_title='".$meta_title."',meta_keyword='".$meta_keyword."',meta_description='".$meta_description."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO brands SET title='".$title."',arabic_title='".$arabic_title."',arabic_meta_title='".$arabic_meta_title."',status='1',brand_id='".$brand_id."',order_no='".$order_no."',alias='".$alias."',meta_title='".$meta_title."',meta_keyword='".$meta_keyword."',meta_description='".$meta_description."'";
			...
			if(isset($_FILES['image']) && $_FILES['image']['error']==0)
			{
				...
				$logofilename = "../uploads/brands/". $brand_image;
				$moveResult = move_uploaded_file($_FILES['image']['tmp_name'],$logofilename);
				$target_file = $logofilename;
				$resized_file = $logofilename;
				$wmax = 229;
				$hmax = 223;
				$resizeResult = resizeImg($target_file,$resized_file,$wmax,$hmax,1);
							...
							<div class="row">
								...
								<div class="col-sm-12">
									<div class="form-group">
										...
										<span class="red" style="color:red;"><font size="2" width="bold">[ Size : 229*223 ]px</font></span>	
									</div> 
								</div>
							</div>
							<div class="form-group">
