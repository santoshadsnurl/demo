...
if(isset($_REQUEST['searchQuery']) && !empty($_REQUEST['searchQuery']))
{
	...
	$strQuery = "WHERE email LIKE '%$searchInput%'";
}

if(isset($_GET['id']) && $_GET['id']!="")
{
	$id = $_GET['id'];
	$query_delete = "DELETE FROM subscribers WHERE id='".$id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM subscribers WHERE id = '".$value."'"))
			{
...
if($sql_count=$conn->query("SELECT * FROM subscribers $strQuery ORDER BY id DESC"))
{
...
$query="SELECT * FROM subscribers $strQuery ORDER BY id DESC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px;" type="text" name="searchQuery" class="form-control" placeholder="Search By Email" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="subscribers.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
						...
						<div class="row">
							<div class="col-sm-4">
								<a class="btn btn-default pull-right" href="newsletter.php">Export</a>
							</div>
							<div class="col-sm-4">
								<a href="compose.php"><button class="btn btn-primary center-block" type="button">Send Mail</button></a>
							</div>
							<div class="col-sm-4">
								<a href="uploadcsv.php"><button class="btn btn-default pull-right" type="button">Upload CSV</button></a>
							</div>
						</div>
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td>
											<a href="subscribers_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="subscribers.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM subscribers $strQuery ORDER BY id DESC $limitvar";
