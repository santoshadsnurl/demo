...
{
	$id = $_GET['id'];
	$category_id = $newobject->getdata($conn,"categories","category_id","id",$id);
	$query_delete="DELETE FROM categories WHERE id='".$id."'";
	if($sql_delete=$conn->query($query_delete))
	{
		$query_delete_sub="DELETE FROM sub_categories WHERE category_id='".$category_id."'";
		$sql_delete_sub=$conn->query($query_delete_sub);
		$query_delete_sub_sub="DELETE FROM sub_sub_categories WHERE category_id='".$category_id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM categories WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM categories $strQuery ORDER BY order_no ASC"))
{
...
$query="SELECT * FROM categories $strQuery ORDER BY order_no ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							...
							<div class="col-sm-4">
								<a href="categories.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td><img src="<?php echo $result['image']; ?>" alt="<?php echo $result['title']; ?>" height="80" width="100"></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('categories','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('categories','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="categories_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="categories.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM categories $strQuery ORDER BY order_no ASC $limitvar";
