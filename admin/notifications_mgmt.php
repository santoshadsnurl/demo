...
if(isset($_REQUEST['searchQuery']) && !empty($_REQUEST['searchQuery']))
{
	...
	$strQuery = "WHERE message LIKE '%$searchInput%'";
}
...
if(isset($_REQUEST['user_id']) && !empty($_REQUEST['user_id']))
{
	...
	if($strQuery!="")
	{
		$strQuery .= " AND firebase_token = '$user_id'";
	}
	else
	{
		$strQuery = "WHERE firebase_token = '$user_id'";
	}
}
...
{
	$id = $_GET['id'];
	$query_delete="DELETE FROM notifications WHERE id='".$id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM notifications WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM notifications $strQuery ORDER BY id DESC"))
{
...
$query="SELECT * FROM notifications $strQuery ORDER BY id DESC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
...
<div class="container-fluid">
	...
	<div class="right-section">
		<form name="pageinfo" id="pageinfo" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
			<input type="hidden" name="user_id" id="user_id" value="<?php if(isset($_GET['user_id'])) { echo $_GET['user_id']; } ?>">
						...
						<div class="row">
							<div class="col-sm-4">
								<div class="input-group">
									<select name="firebase_token" id="firebase_token" class="form-control" onchange="categories(this.value);">
									<option value="">--Select--</option>
									<?php
									$query_cat = "SELECT * FROM users WHERE status = 1 AND fcm_token!='' ORDER BY id DESC";
									if($stmt_cat = $conn->query($query_cat))
									{
										while($r_cat = $stmt_cat->fetch_array(MYSQLI_ASSOC))
										{
										?>
										<option value="<?php echo $r_cat['fcm_token'];?>" <?php if(isset($_GET['user_id']) && $_GET['user_id']==$r_cat['fcm_token']) { echo "selected"; } ?>><?php echo ucwords($r_cat['name']);?></option> 	
										<?php 
										} 
									} 
									?>
						...
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px; " type="text" name="searchQuery" class="form-control" placeholder="Search By Message" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="notifications.php"><button class="btn btn-primary center-block" type="button">Send Notification</button></a>
							</div>
							<div class="col-sm-4">
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td><?php echo $newobject->getdata($conn,"users","name","fcm_token",$firebase_token); ?></td>
											<td><?php echo $result['title']; ?></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('notifications','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('notifications','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="notifications_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											</td>
											<th>
...
{
	//alert(category_id);
	document.getElementById('user_id').value = category_id;
...
<?php  
$ban="SELECT * FROM notifications $strQuery ORDER BY id DESC $limitvar";
