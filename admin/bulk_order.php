...
if(isset($_REQUEST['searchQuery']) && !empty($_REQUEST['searchQuery']))
{
	...
	$strQuery = "WHERE name LIKE '%$searchInput%'";
}

if(isset($_GET['id']) && $_GET['id']!="")
{
	$id = $_GET['id'];
	$query_delete = "DELETE FROM bulk_order WHERE id='".$id."'";
	if($sql_delete = $conn->query($query_delete))
	{
		$_SESSION['response'] = "Deleted Successfully.";
		echo "<script>document.location.href='bulk_order.php';</script>";
	    exit();					
	}
}
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM bulk_order WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM bulk_order $strQuery ORDER BY id ASC"))
{
...
$query="SELECT * FROM bulk_order $strQuery ORDER BY id ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
		...
		<div class="main-section">
			...
			<div class="col-sm-12 col-md-12 col-xs-12">
				<div class="panel panel-default">
					...
					<h3 class="panel-title">Bluk Order</h3>
					</div>
							...
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px;" type="text" name="searchQuery" class="form-control" placeholder="Search By User Name" value="<?php echo $searchInput;?>"  required/>
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td><?php echo $newobject->getInfo($conn,"products","title",$result['product_id']); ?></td>
											<td><?php echo $result['enq_quantity']; ?></td>
...
<?php  
$ban="SELECT * FROM bulk_order $strQuery ORDER BY id $limitvar";
