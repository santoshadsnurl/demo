...
if(isset($_REQUEST['searchQuery']) && !empty($_REQUEST['searchQuery']))
{
	...
	$strQuery = "WHERE (title LIKE '%$searchInput%' || sku LIKE '%$searchInput%')";
}
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM products WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM products $strQuery ORDER BY id DESC"))
{
...
echo $query="SELECT * FROM products $strQuery ORDER BY id DESC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px; " type="text" name="searchQuery" class="form-control" placeholder="Search By Products" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="products.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
								<a href="uploadcsvp.php"><button class="btn btn-default pull-right" style="margin-left:10px" type="button">Upload CSV</button></a>
							...
							if($sql_select=$conn->query($query))
							{
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										$brand = $newobject->getdata($conn,"brands","title","id",$brand_id);
										$ctr++;
										?>
										<tr>
											<td scope="row"><?php echo $ctr; ?></td>
											<td><?php echo $category; ?></td>
											<td><?php echo $result['title']; ?></td>
											<td><a href="<?php echo $site_root; ?>detail/<?php echo $alias; ?>/" target="_blank">Preview</a></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('products','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('products','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="products_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="products.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<td>
								...
								{
									?>
									<td  colspan="7" align="center"><font color="red" size="2">Record Not Found</font></td>
									<?php
								}
							}
							?>
...
<?php  
$ban="SELECT * FROM products $strQuery ORDER BY id DESC $limitvar";
