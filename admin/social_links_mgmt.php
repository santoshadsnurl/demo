...
{
	$id = $_GET['id'];
	$query_delete = "DELETE FROM social_links WHERE id='".$id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM social_links WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM social_links $strQuery ORDER BY id ASC"))
{
...
$query="SELECT * FROM social_links $strQuery ORDER BY id ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							...
							<div class="col-sm-4">
								<a href="social_links.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
								...
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										<tr>
											...
											<td><img src="<?php echo $result['image']; ?>" alt="<?php echo $result['title']; ?>" height="40px" width="40px"></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('social_links','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('social_links','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="social_links_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="social_links.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM social_links $strQuery ORDER BY id $limitvar";
