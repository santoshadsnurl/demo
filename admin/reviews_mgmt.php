...
if(isset($_REQUEST['searchQuery']) && !empty($_REQUEST['searchQuery']))
{
	...
	$query_select = "SELECT id FROM products WHERE title ='".$searchInput."'";
	if($sql_select = $conn->query($query_select))
	{
		if($sql_select->num_rows>0)
		{
			...
			$strQuery = "WHERE product_id = '$product_id'";
		}
	}
}
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM reviews WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM reviews $strQuery ORDER BY id DESC"))
{
...
$query="SELECT * FROM reviews $strQuery ORDER BY id DESC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px;" type="text" name="searchQuery" class="form-control" placeholder="Search By Product" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="reviews.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
							...
							if($sql_select=$conn->query($query))
							{
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										$product_title = $newobject->getInfo($conn,"products","title",$product_id);
										$name = $newobject->getInfo($conn,"users","name",$user_id);
										$ctr++;
										?>
										<tr>
											...
											<td><?php echo $name; ?></td>
											<td><?php echo $result['reviews']; ?></td>
											<td><?php echo @date("Y-m-d",strtotime($result['date'])); ?></td>
											<td><?php echo $result['rating']; ?></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('reviews','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('reviews','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="reviews_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="reviews.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM reviews $strQuery ORDER BY id DESC $limitvar";
