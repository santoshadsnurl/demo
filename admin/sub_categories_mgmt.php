...
{
	$id = $_GET['id'];
	$sub_category_id = $newobject->getdata($conn,"sub_categories","sub_category_id","id",$id);
	$query_delete="DELETE FROM sub_categories WHERE id='".$id."'";
	if($sql_delete=$conn->query($query_delete))
	{
		
		$query_delete_sub_sub="DELETE FROM sub_sub_categories WHERE sub_category_id='".$sub_category_id."'";
		$sql_delete_sub_sub=$conn->query($query_delete_sub_sub);
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{	
			if($sql_deleteall=$conn->query("DELETE FROM sub_categories WHERE id = '".$value."'"))
			{
				$_SESSION['response'] = "Deleted Successfully.";				
			}
	    }
...
if($sql_count=$conn->query("SELECT * FROM sub_categories $strQuery ORDER BY order_no ASC"))
{
...
$query="SELECT * FROM sub_categories $strQuery ORDER BY order_no ASC $limitvar";
$start_1 =($st+1);
$start =$st;
$end =($start+$pageshow);
$page="";
...
<div class="container-fluid">
	...
	<div class="right-section">
		...
		<div class="alert alert-success fade in" style="margin:10px 10px 10px 10px;">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								<form name="searchform" method="get" action="<?php echo $_SERVER['REQUEST_URI'];?>">
									<div class="input-group">
										<input style="border:1px solid #d3d3d3; height:38.5px;" type="text" name="searchQuery" class="form-control" placeholder="Search By Sub Categories" value="<?php echo $searchInput;?>"  required/>
							...
							<div class="col-sm-4">
								<a href="sub_categories.php"><button class="btn btn-primary center-block" type="button">Add New</button></a>
							</div>
							<div class="col-sm-4">
							...
							if($sql_select=$conn->query($query))
							{
								if($sql_select->num_rows>0)
								{
									...
									while($result=$sql_select->fetch_array(MYSQLI_ASSOC))
									{
										...
										$category = $newobject->getdata($conn,"categories","title","id",$category_id);
										$ctr++;
										?>
										<tr>
											<th scope="row"><?php echo $result['order_no']; ?></th>
											<td><?php echo $category; ?></td>
											<td><?php echo $result['title']; ?></td>
											<td>
											<?php if($result['status'] == 1) { ?>
											<input type="checkbox" onclick="return chkstatus('sub_categories','0','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>" checked / >
											<?php }	else { ?>
											<input type="checkbox"  onclick="return chkstatus('sub_categories','1','<?php echo $id;?>');" name="status<?php echo $id; ?>" id="status<?php echo $id; ?>" value="<?php echo $result['status'];?>"/ >
											<?php }	?>
											</td>
											<td>
											<a href="sub_categories_mgmt.php?id=<?php echo $result['id']; ?>" title="Delete" class="btn btn-primary btn-xs" onClick="return confirm('Are you sure to delete?');"> <i class="glyphicon glyphicon-trash"></i> </a>
											<a href="sub_categories.php?id=<?php echo $result['id']; ?>" title="Edit" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
											</td>
											<th>
...
<?php  
$ban="SELECT * FROM sub_categories $strQuery ORDER BY order_no ASC $limitvar";
